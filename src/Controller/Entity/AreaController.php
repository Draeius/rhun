<?php

namespace App\Controller\Entity;

use App\Controller\BasicController;
use App\Entity\Area;
use App\Form\AreaType;
use App\Repository\AreaRepository;
use App\Service\ParamGenerator\EditorParamGenerator;
use Symfony\Component\HttpFoundation\Request;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\Routing\Annotation\Route;

/**
 * @Route("/area")
 * @App\Annotation\Security(needAccount=true, editWorld=true)
 */
class AreaController extends BasicController {

    /**
     *
     * @var EditorParamGenerator
     */
    private $paramGenerator;

    function __construct(EditorParamGenerator $paramGenerator) {
        $this->paramGenerator = $paramGenerator;
    }

    /**
     * @Route("/", name="area_index", methods={"GET"})
     */
    public function index(AreaRepository $areaRepo): Response {
        return $this->render($this->getSkinFile(), array_merge($this->paramGenerator->getEditorParams('editors/area/index'), [
                    'areas' => $areaRepo->findAll()
        ]));
    }

    /**
     * @Route("/new", name="area_new", methods={"GET","POST"})
     */
    public function new(Request $request): Response {
        $area = new Area();
        $form = $this->createForm(AreaType::class, $area);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $entityManager = $this->getDoctrine()->getManager();
            $entityManager->persist($area);
            $entityManager->flush();

            return $this->redirectToRoute('area_index');
        }
        
        return $this->render($this->getSkinFile(), array_merge($this->paramGenerator->getEditorParams('editors/area/new'), [
                    'area' => $area,
                    'form' => $form->createView()
        ]));
    }

    /**
     * @Route("/{id}", name="area_show", methods={"GET"})
     */
    public function show(Area $area): Response {
        return $this->render($this->getSkinFile(), array_merge($this->paramGenerator->getEditorParams('editors/area/show'), [
                    'area' => $area
        ]));
    }

    /**
     * @Route("/{id}/edit", name="area_edit", methods={"GET","POST"})
     */
    public function edit(Request $request, Area $area): Response {
        $form = $this->createForm(AreaType::class, $area);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('area_index', [
                        'id' => $area->getId(),
            ]);
        }
        return $this->render($this->getSkinFile(), array_merge($this->paramGenerator->getEditorParams('editors/area/edit'), [
                    'area' => $area,
                    'form' => $form->createView()
        ]));
    }

    /**
     * @Route("/{id}", name="area_delete", methods={"DELETE"})
     */
    public function delete(Request $request, Area $area): Response {
        if ($this->isCsrfTokenValid('delete' . $area->getId(), $request->request->get('_token'))) {
            $entityManager = $this->getDoctrine()->getManager();
            $entityManager->remove($area);
            $entityManager->flush();
        }

        return $this->redirectToRoute('area_index');
    }

}
